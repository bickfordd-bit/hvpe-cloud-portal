...
const opportunity: Opportunity = {
  ...
  links: ['https://www.example.com/rfi-cyber-security', 'https://www.example.com/technical-specs'],
  documents: [
    {
      id: 'doc-001',
...
console.log(
  `   Documents: ${opportunity.links?.length || 0} links, ${opportunity.documents?.length || 0} attachments`
);
...
async function runDemo() {
  try {
    ...
    const shallMet = result.traces.filter(
      (t, i) => requirements[i].kind === 'shall' && t.confidence >= 0.7
    );
    console.log(
      `   Mandatory Compliance: ${shallMet.length}/${shallReqs.length} "shall" requirements met`
    );
    ...
    console.log(
      `   Result: ${Math.pow(weightedCoverage, 0.8).toFixed(3)} × ${Math.pow(avgConfidence, 0.2).toFixed(3)} × ${(shallMet.length / shallReqs.length).toFixed(3)} × 1.3`
    );
