...
const requiredCommands = ["DEFINE", "GAP", "FREEZE", "SIM", "SCORE", "OPTR", "T2V", "LEDGER", "PROOF", "SHIP"];
const commandIds = spec.defines.commands.map((c: any) => c.id);

for (const cmd of requiredCommands) {
  if (commandIds.includes(cmd)) {
    const c = spec.defines.commands.find((x: any) => x.id === cmd);
...
if (spec.trading_controls.billy.live_guardrail.require_env_flag === "ALLOW_BILLY_LIVE_TRADING") {
  console.log(`   ✓ Billy live guardrail: ${spec.trading_controls.billy.live_guardrail.require_env_flag}`);
} else {
