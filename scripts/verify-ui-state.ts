...
check(
  ...
  () => {
    const file = fs.readFileSync(
      path.join(WORKSPACE, "src/lib/licenseSession.crypto.ts"),
      "utf8"
    );
    return (
      file.includes("export async function getSession()") &&
      file.includes("await cookies()")
    );
  },
...
check(
  ...
  () => {
    const file = fs.readFileSync(
      path.join(WORKSPACE, "src/app/page.tsx"),
      "utf8"
    );
...
check(
  ...
  () => {
    const file = fs.readFileSync(
      path.join(WORKSPACE, "src/app/t/jake/page.tsx"),
      "utf8"
    );
    return (
...
check(
  ...
  () => {
    ...
    const oneStatus =
      (file.match(/status/gi) || []).length <= 2; // Appears in type def + usage
    return (
      noMetrics &&
      noCharts &&
      oneHeader &&
      oneStatus &&
      file.includes("Decision Continuity")
    );
  },
...
check(
  ...
  () => {
    const file = fs.readFileSync(
      path.join(WORKSPACE, "src/app/t/billy/page.tsx"),
      "utf8"
    );
    return (
...
check(
  ...
  () => {
    const jakePage = fs.readFileSync(
      path.join(WORKSPACE, "src/app/t/jake/page.tsx"),
      "utf8"
    );
    const billyPage = fs.readFileSync(
      path.join(WORKSPACE, "src/app/t/billy/page.tsx"),
      "utf8"
    );
    return (
...
check(
  ...
  () => {
    ...
    return (
      !file.includes('"use client"') &&
      file.includes("export default async function Home()")
    );
  },
...
console.log("\n" + "=".repeat(70));
console.log(
  `Results: ${passed}/${checks.length} passed${failed > 0 ? `, ${failed} failed` : ""}`
);
console.log("=".repeat(70) + "\n");
