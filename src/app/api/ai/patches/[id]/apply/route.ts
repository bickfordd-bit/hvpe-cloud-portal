...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  // require approval
  if (!patchLog.approvedAt) return NextResponse.json({ error: "patch not approved" }, { status: 403 });
  ...
  try {
    ...
    execSync(`git commit -m "AI approved: ${patchLog.instruction.slice(0, 120).replace(/\"/g, "'")}"`, { stdio: "inherit" });
    try { execSync(`git push origin HEAD:${branch}`, { stdio: "inherit" }); } catch (e) {}
    ...
    await prisma.aIPatchLog.update({ where: { id }, data: { applied: true, branch, prUrl: prUrl || undefined } });
