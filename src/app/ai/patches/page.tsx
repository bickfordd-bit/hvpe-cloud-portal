            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-xs text-neutral-400">
                  {p.approvedAt
                    ? `Approved → ${new Date(p.approvedAt).toLocaleString()}`
                    : 'Not approved'}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="text-xs text-neutral-400">
                  {p.applied ? `Applied → ${p.branch}` : 'Pending'}
                </div>
                {!p.applied && <ApprovePatchButton id={p.id} />}
              </div>
            </div>
            <pre className="mt-2 text-xs overflow-auto max-h-72 text-neutral-200 bg-black p-2 rounded">
              {p.patch}
            </pre>
          </div>
        ))}
