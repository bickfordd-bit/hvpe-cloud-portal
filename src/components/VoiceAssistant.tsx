...
export default function VoiceAssistant() {
  ...
  useEffect(() => {
    // @ts-expect-error - SpeechRecognition types not available in all browsers
    ...
    <div className="fixed right-6 bottom-6 z-50">
      <div className="bg-neutral-900/80 backdrop-blur-md rounded-lg p-3 w-72">
        <div className="flex items-center gap-2">
          ...
          <button className="px-2 py-1 rounded bg-blue-600" onClick={() => handleSend(true)}>
            Apply
          </button>
          <button className="px-2 py-1 rounded bg-slate-600" onClick={() => handleSend(false)}>
            Preview
          </button>
        </div>
        <div className="mt-2 text-xs text-neutral-300">
          <div className="h-12 overflow-auto whitespace-pre-wrap">
            {transcript || <em>Say a command or paste text</em>}
          </div>
