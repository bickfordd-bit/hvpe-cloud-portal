...
function calculateBillionairePath(
  totalValue: number,
  years: number
): {
...
export function BillionaireTracker({
  ...
  // Initialize with server-safe default, will hydrate to client time
  const [currentTime, setCurrentTime] = useState<number>(() =>
    typeof window === 'undefined' ? new Date('2025-12-14T00:00:00Z').getTime() : Date.now()
  );
        ...
        <div>
          ...
          <div className="mt-1 text-[11px] text-neutral-400">
            ...
            <span className="text-neutral-100 font-semibold">
              ${projection.totalValue.toLocaleString('en-US')}
            </span>
          </div>
          <div className="text-[11px] text-neutral-400">
            Timeline to Complete:{' '}
            <span className="text-neutral-100 font-semibold">
              {projection.timelineYears.toFixed(1)} years
            </span>
          </div>
          <div className="text-[11px] text-neutral-400">
            Billionaire Path:{' '}
            <span
              className={`font-semibold ${billionairePath.isAchievable ? 'text-emerald-400' : 'text-orange-400'}`}
            >
          ...
          <div className="text-[10px] text-neutral-500">{confidence.commentary}</div>
        </div>
        <div className="text-[11px] text-neutral-500 text-right">{description}</div>
      </div>
          ...
          return (
            <div key={p.name} className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${getStatusColor(p.status)} bg-current/10`}
                >
                  {p.status.replace('-', ' ')}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between text-[10px] text-neutral-500">
                ...
                <div className="text-right">
                  <div>
                    Days Until Sale:{' '}
                    <span
                      className={`text-neutral-200 ${isOverdue ? 'text-red-400' : daysUntil < 30 ? 'text-yellow-400' : ''}`}
                    >
