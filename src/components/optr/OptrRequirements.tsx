          ...
          requirements.map((r) => (
            <div key={r.id} className="rounded-xl border border-neutral-800 bg-neutral-950 p-3">
              <div className="flex items-center justify-between gap-3">
                <div className="text-xs text-neutral-500">{(r as any).section || "—"}</div>
                <div className="text-xs uppercase tracking-widest text-neutral-400">{(r as any).kind || r.priority}</div>
              </div>
